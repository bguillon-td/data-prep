...
class PipelineConsoleDump implements Visitor {

    private final StringBuilder builder;

    int indent;

    PipelineConsoleDump(StringBuilder builder) {
    ...
    public void visitCompile(CompileNode compileNode) {
        builder.append("COMPILE").append(" [").append(compileNode.getAction().getName()).append("] ").append(" (executed: ")
                .append(compileNode.executed()).append(')').append('\n');
    }
    ...
    @Override
    public void visitWriterLink(WriterLink writerLink) {
        builder.append("-> OUT (").append(writerLink.getWriter()).append(")");
    }
